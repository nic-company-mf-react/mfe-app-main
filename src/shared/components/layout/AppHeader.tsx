	...
	return (
		<header className="sticky top-0 flex w-full bg-white border-b border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900">
			<div className="flex items-center justify-between w-full gap-2 px-3 py-3 sm:gap-4 lg:px-6 lg:py-4">
				...
				{/* 언어 전환 + 다크모드 토글 + Github 버튼 - 항상 우측 표시 */}
				<div className="flex shrink-0 items-center gap-2 ml-auto lg:ml-0">
					<button
						onClick={toggleLanguage}
						aria-label="Toggle Language"
						className="flex items-center justify-center gap-1 h-10 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-600 transition-colors hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 lg:h-11"
					>
						<span
							className={
								currentLang === 'ko' ? 'text-brand-500 dark:text-brand-400' : 'text-gray-400 dark:text-gray-500'
							}
						>
							KO
						</span>
						...
						<span
							className={
								currentLang === 'en' ? 'text-brand-500 dark:text-brand-400' : 'text-gray-400 dark:text-gray-500'
							}
						>
							EN
						</span>
